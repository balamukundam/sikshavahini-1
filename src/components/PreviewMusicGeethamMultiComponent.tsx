...
const PreviewMusicGeethamMultiComponent = ({
  musicGeethamMultiComp,
  musicSettings,
  talamShow,
  stopPlayClicked,
  rowg,
  ...
  const initialize = () => {
    setRagam(musicGeethamMultiComp["ragam"]);
    ...
    let eventsAndRwowsPallavi: any = [];
    if (musicGeethamMultiComp["musicPallavi"] !== "") {
      ms.getNotesRaw(musicGeethamMultiComp["musicPallavi"]);
      eventsAndRwowsPallavi = getEvents(
        ms.getNoteNbrsArray(),
        ms.getNoteTextArray(),
      ...
      tempTableHeaders.push("Pallavi");
      tempNotesPerBeatForTable.push(musicGeethamMultiComp["pallaviGati"]);
      eventstemp.push(eventsAndRwowsPallavi.events);
      tablestemp.push(eventsAndRwowsPallavi.rows);
      totalEvents = totalEvents + eventsAndRwowsPallavi.events.length;
    }

    musicGeethamMultiComp["musicLyrics"].forEach(
      (charanamLine: string, index: number) => {
        ms.getNotesRaw(musicGeethamMultiComp["musicNotes"][index]);

        let nbp: number = 4;
        if (checkedTalamNumbers.length > 0) nbp = checkedTalamNumbers[0];
        if (checkedTalamNumbers.length > index)
          nbp = checkedTalamNumbers[index];
  ...
  useEffect(() => {
    initialize();
  }, [musicGeethamMultiComp, musicSettings]);
  ...
  if (checkValidTalamNumbers(musicGeethamMultiComp.talamSeq, 0, 6)) {
    images = musicGeethamMultiComp.talamSeq
      .split(",") // Split by comma
  ...
  let checkedTalamNumbers: number[] = musicGeethamMultiComp.gatis;
...
export default PreviewMusicGeethamMultiComponent;
